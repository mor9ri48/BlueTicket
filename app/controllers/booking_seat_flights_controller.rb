class BookingSeatFlightsController < ApplicationController
  ...
  def new
    ...
    # 座席選択の条件を指定
    if params[:passenger_seat].present?
      ...
      redirect_to request.referer, notice: "座席が選択されていません。"
    end
  end
  ...
  def create
    ...
    @seats = Array.new(params[:number_of_passengers].to_i)
    seat_exist = false
    for number in 0..params[:number_of_passengers].to_i-1 do
      @seats[number] = Seat.find_by(airmodel_id: params[:airmodel_id].to_i, number: params[:passenger_seat][number])
      if BookingSeatFlight.where("flight_id = ? AND seat_id = ?", @flight.id, @seats[number].id).present?
        seat_exist = true
      end
    end
    if seat_exist
