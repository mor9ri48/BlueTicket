...
0.upto(30) do                           #31日分データ作成
  ...
  10.upto(39) do |flightnum|  #便番号の下二桁
    ...
    if (orig_dest_ids.include?(1)) then                                     #新千歳←→
      if (orig_dest_ids.include?(2) || orig_dest_ids.include?(3) || orig_dest_ids.include?(4)) then    #羽田・成田・中部
        price = 8000
        duration = 120
      ...
      if (orig_dest_ids.include?(4) || orig_dest_ids.include?(5) || orig_dest_ids.include?(6)) then    #中部・関空・伊丹
        price = 7000
        duration = 75
      elsif (orig_dest_ids.include?(7)) then                              #福岡
        price = 10000
